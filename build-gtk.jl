...
(defun output-complex-type (type typage)
  ...
  (if (or (assq type gtk-enums) (assq type gtk-imported-enums)
      ...
      (symbol-name type)
    (if (or (string-match "Func" (format nil "%s" type))
            (string-match "Destroy" (format nil "%s" type))
            (string-match "Foreach" (format nil "%s" type)))
        (format nil "%s" type)
      (format nil "%s*" type))))
...
(defun output-rep-to-boxed (output type rep-var typage)
  (declare (unused typage))
  (setq type (gtk-outer-type type))
  (if (or (string-match "func" (format nil "%s" rep-var))
          (string-match "destroy" (format nil "%s" rep-var))
          (string-match "notify" (format nil "%s" rep-var))
          (string-match "foreach" (format nil "%s" rep-var)))
